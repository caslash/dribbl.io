        ...
        _ = self.connectionPool.withConnection { connection in
            ...
            for player in players {
                bindings.append(contentsOf: [
                    ...
                    PostgresData(string: self.dateFormatter.string(from: player.birthdate)),
                    PostgresData(string: player.school),
                    PostgresData(string: player.country),
                    PostgresData(string: player.height),
                    PostgresData(int: player.weight),
                    PostgresData(int: player.season_exp),
                    player.jersey.map { PostgresData(int: $0) } ?? PostgresData.null,
                    PostgresData(string: player.position),
                    PostgresData(string: player.team_history),
                    PostgresData(bool: player.is_active),
                    PostgresData(int: player.from_year),
                    PostgresData(int: player.to_year),
                    PostgresData(int: player.total_games_played),
                    PostgresData(string: player.draft_round),
                    PostgresData(string: player.draft_number),
                    PostgresData(string: player.draft_year),
                    PostgresData(double: player.career_ppg),
                    PostgresData(double: player.career_rpg),
                    PostgresData(double: player.career_apg)
                ])
            }
