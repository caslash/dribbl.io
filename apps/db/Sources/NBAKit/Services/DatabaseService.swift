...
public final class DatabaseService: DatabaseServiceProtocol {
    ...
    public init(config: DatabaseConfig, logger: String = "DatabaseService") throws {
        ...
        self.logger.info(
            "DatabaseService initialized with max connections: \(config.maxConnections), batch size: \(config.batchSize)"
        )
    }
    ...
    public func batchInsertPlayers(_ players: [Player]) async throws {
        ...
        for (batchIndex, batch) in batches.enumerated() {
            do {
                try await self.insertPlayerBatch(
                    batch, batchIndex: batchIndex + 1, totalBatches: batches.count)
                successCount += batch.count
                self.logger.debug(
                    "Successfully inserted batch \(batchIndex + 1)/\(batches.count) with \(batch.count) players"
                )
            } catch {
        ...
        logger.info(
            "Player batch insert completed: \(successCount) successful, \(failureCount) failed")
    }

    private func insertPlayerBatch(_ players: [Player], batchIndex: Int, totalBatches: Int)
        async throws
    {
        try await self.connectionPool.withConnection { connection in
            let sql = """
                INSERT INTO player (
                    id, first_name, last_name, display_first_last, birthdate, school, country, height, weight,
                ...
                + players.enumerated().map { index, _ in
                    let baseParam = index * 23 + 1
                    return
                        "($\(baseParam), $\(baseParam+1), $\(baseParam+2), $\(baseParam+3), $\(baseParam+4), $\(baseParam+5), $\(baseParam+6), $\(baseParam+7), $\(baseParam+8), $\(baseParam+9), $\(baseParam+10), $\(baseParam+11), $\(baseParam+12), $\(baseParam+13), $\(baseParam+14), $\(baseParam+15), $\(baseParam+16), $\(baseParam+17), $\(baseParam+18), $\(baseParam+19), $\(baseParam+20), $\(baseParam+21), $\(baseParam+22))"
                }.joined(separator: ", ") + """
            ...
            for player in players {
                bindings.append(contentsOf: [
                    ...
                    PostgresData(string: player.display_first_last),
    ...
    public func batchInsertPlayerAccolades(_ accolades: [PlayerAccolades]) async throws {
        ...
        for (batchIndex, batch) in batches.enumerated() {
            do {
                try await insertPlayerAccoladesBatch(
                    batch, batchIndex: batchIndex + 1, totalBatches: batches.count)
                successCount += batch.count
                logger.debug(
                    "Successfully inserted accolades batch \(batchIndex + 1)/\(batches.count) with \(batch.count) records"
                )
            } catch {
                failureCount += batch.count
        ...
        self.logger.info(
            "Player accolades batch insert completed: \(successCount) successful, \(failureCount) failed"
        )
    }

    private func insertPlayerAccoladesBatch(
        _ accolades: [PlayerAccolades], batchIndex: Int, totalBatches: Int
    ) async throws {
        try await self.connectionPool.withConnection { connection in
            let sql = """
