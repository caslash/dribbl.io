...
public final class DataProcessor {
    ...
    private func processBatch(_ players: [PlayerInfo]) async -> (players: [Player], accolades: [PlayerAccolades]) {
        ...
        for info in players {
            do {
                let (player, accolade) = try await processPlayer(info)
    ...
    private func processPlayer(_ playerInfo: PlayerInfo) async throws -> (Player, PlayerAccolades) {
        ...
        let cpi: CommonPlayerInfo = try await self.nbaApiService.fetchWithRetry(endpoint: .commonPlayerInfo, playerId: playerInfo.id)
        
        let profile: PlayerProfileV2 = try await self.nbaApiService.fetchWithRetry(endpoint: .playerProfileV2, playerId: playerInfo.id)
        
        let awards: PlayerAwardsList = try await self.nbaApiService.fetchWithRetry(endpoint: .playerAwards, playerId: playerInfo.id)
    ...
    private func getPlayerTeamHistory(regularSeasonTotals: [SeasonTotalsRegularSeason]) -> String {
        ...
        let uniqueTeamIds = onlyNBASeasons.map { $0.playerId }.unique().map { String($0) }
